...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','chiwuzhe');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','chiwuzhe');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('chiwuzhe', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusschiwuzhe/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
