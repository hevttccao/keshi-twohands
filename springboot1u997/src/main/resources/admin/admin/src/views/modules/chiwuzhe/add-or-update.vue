...
export default {
	data() {
		...
		return {
			...
			rules: {
				chiwuzhanghao: [
					{ required: true, message: '持物账号不能为空', trigger: 'blur' },
				],
				chiwuxingming: [
					{ required: true, message: '持物姓名不能为空', trigger: 'blur' },
				],
				mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chiwuzhe/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chiwuzheCrossAddOrUpdateFlag = false;
						 this.parent.search();
